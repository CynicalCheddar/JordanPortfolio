...
#define SYS_CHECK     ( 0x09 )
#define SYS_SEMREAD   ( 0x10 )
#define SYS_MAKECHANNEL   ( 0x11 )
#define SYS_CHANNELCONTROLLER   ( 0x12 )
#define SYS_REQPID   ( 0x13 )
#define SYS_WRITE_GRAPHIC     ( 0x14 )
#define SYS_DRAW_RECT     ( 0x15 )
...
// for process identified by pid, set send produce to x
extern void send( pid_t pid, int x );


// Checks buffer. Return 0 if empty. Return 1 if part filled. Return 2 if full.
extern int checkBuffer( pid_t pid);

// for process identified by pid, return the next value from its semaphore queue.
extern int semRead( pid_t pid);

// Make a communication channel between two processes. Return 1 on success.
extern void makeChannel(pid_t pidSource, pid_t pidDest);

// Reads the command sent to the channel controller. May add or remove from broadcast pids.
extern int channelControlRead();

extern int requestPid();

// write n bytes from x to   the file descriptor fd; return bytes written
extern void graphicWrite( int box, const void* x, size_t n);

extern void drawRectangleCall(int y, int x, int height, int width);
